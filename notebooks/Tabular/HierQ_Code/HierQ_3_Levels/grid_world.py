...
class Grid_World:
    ...
    # Reset grid world visualization
    ...
    # Visualize Subgoals
    def display_subgoals(self, subgoal_1, old_subgoal_1, subgoal_2, old_subgoal_2, state, goal):

        if subgoal_2 != old_subgoal_2:
            if old_subgoal_2 != -1:
                # If agent currently at old subgoal
                if old_subgoal_2 == state:
                    self.canvas.itemconfig(old_subgoal_2 + 1, fill="blue")
                elif old_subgoal_2 == goal:
                    self.canvas.itemconfig(old_subgoal_2 + 1, fill="yellow")
                else:
                    self.canvas.itemconfig(old_subgoal_2 + 1, fill="white")

            self.canvas.itemconfig(subgoal_2 + 1, fill="green")
