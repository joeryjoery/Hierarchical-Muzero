...
class TabularHierarchicalAgent(Agent, ABC):
    ...
    def __init__(self, observation_shape: typing.Tuple, n_actions: int, n_levels: int) -> None:
        super().__init__(observation_shape=observation_shape, n_actions=n_actions)
        assert n_levels > 1, f"Attempting to initialize an Hierarchical Agent conflicting: n_levels > 1, got {n_levels}"
