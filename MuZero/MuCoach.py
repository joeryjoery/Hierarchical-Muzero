...
from MuZero.MuMCTS import MuZeroMCTS, MuZeroContinuousMCTS
...
class MuZeroCoach(Coach):
    ...
    def buildHypotheticalSteps(self, history: GameHistory, t: int, k: int) -> \
        ...
        :return: Tuple of (actions, targets, future_inputs) that the neural network needs for optimization OR
        """
        ...
        # (Actions, Targets, Observations, action_spaces)
...
class ContinuousMuZeroCoach(Coach):

    def __init__(self, game, neural_net, args: DotDict, run_name: typing.Optional[str] = None) -> None:
        """
        TODO: Doc.
        """
        super().__init__(game, neural_net, args, MuZeroContinuousMCTS, DefaultMuZeroPlayer)
    ...
    def buildHypotheticalSteps(self, history: GameHistory, t: int, k: int) -> \
        ...
        :param k: int The number of unrolling steps to perform/ length of the dynamics model target sequence.
        :return: Tuple of (actions, targets, future_inputs) that the neural network needs for optimization OR
                 Tuple of (actions, targets
        """
        # One hot encode actions.
        actions = history.actions[t:t+k]
        a_truncation = k - len(actions)
        if a_truncation > 0:  # Uniform policy when unrolling beyond terminal states.
            actions += [np.random.uniform(size=self.game.getActionSize()) for _ in range(a_truncation)]

        # Value targets.
        params = history.probabilities[t:t+k+1]  # ==> Parameterized distributional parameters.
        vs = history.observed_returns[t:t+k+1]
        rewards = history.rewards[t:t+k+1]

        # Actions opened during Progressive Widening.
        action_support = history.opened_actions[t:t+k+1]

        # Handle truncations > 0 due to terminal states. Treat last state as absorbing state
        t_truncation = (k + 1) - len(params)  # Target truncation due to terminal state
        if t_truncation > 0:
            action_support += [np.random.uniform(size=action_support[-1].shape) for _ in range(t_truncation)]
            params += [np.zeros_like(params[-1])] * t_truncation  # Zero vector
            rewards += [0] * t_truncation                   # = 0
            vs += [0] * t_truncation                        # = 0
        ...
        # (Actions, Targets, Observations, action_spaces)
        return actions, (np.asarray(vs), np.asarray(rewards), np.asarray(params)), obs_trajectory, action_support
