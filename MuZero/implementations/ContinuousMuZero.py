...
from tensorflow import GradientTape, clip_by_norm

from utils.loss_utils import support_to_scalar, scalar_to_support, cast_to_tensor
from MuZero.MuNeuralNet import MuZeroContinuousNeuralNet
import Agents
...
class ContinuousMuZero(MuZeroContinuousNeuralNet):
    """
    ...
    def __init__(self, game, net_args: DotDict, architecture: str) -> None:
        ...
        """
        super().__init__(game, net_args, Agents.ContinuousMuZeroNetworks[architecture])
    ...
    def train(self, examples: typing.List) -> float:
        """ TODO: Doc. Only thing that changed is that 'action_support' is now included.
        ...
        observations, actions, targets, forward_observations, action_support, sample_weight = list(zip(*examples))
    ...
    def initial_inference(self, observations: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, float]:
        ...
        s_0, params, v = self.neural_net.forward.predict(observations)

        # Cast bins to scalar
        v_real = support_to_scalar(v, self.net_args.support_size)

        return s_0[0], params[0], np.ndarray.item(v_real)

    def recurrent_inference(self, latent_state: np.ndarray, action: typing.Union[np.ndarray, tuple]) -> \
            typing.Tuple[float, np.ndarray, np.ndarray, float]:
        """ TODO: Rework Doc
        ...
        r, s_next, params, v = self.neural_net.recurrent.predict([latent_state, a_plane])

        # Cast bins to scalar
        v_real = support_to_scalar(v, self.net_args.support_size)
        r_real = support_to_scalar(r, self.net_args.support_size)

        return np.ndarray.item(r_real), s_next[0], params[0], np.ndarray.item(v_real)
